...
select 
    ...
    store.store_name + ', ' +store.store_city + ' (' + store.store_state + ')' as staff_store_adresss,
    manager.first_name + ' ' + manager.last_name as manager_name,
    manager.email as manager_email,
    manager.phone as manager_phone,
    manager.active as manager_active,
    store2.store_name + ', ' +store2.store_city + ' (' + store2.store_state + ')' as manager_store_adresss
from 
    {{ ref('stg__staffs') }} staff
left join {{ ref('stg__staffs') }} manager on manager.manager_id = staff.staff_id
